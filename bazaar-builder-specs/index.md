...
| **L3** | [**L3-controller.md**](L3-controller.md) | review | `bazaar-issues.sh` / `bazaar-build.sh` over `lib/bazaar-common.sh`: intake = no `bzr-*` label, pid-held claims (no TTL), three-attempt escalation, sweeps (dead-pid, bounce, approval, rejected spec, merged PR), optional Haiku tie-break |
...
- **Phase 1 done 2026-09-19:** `lib/bazaar-review.sh` extracted, `test-bazaar-review-lib.sh` 59/59 green. Phase 2 (controllers) is next.
...
3. Phases 2-4 in order; each lands with its L3's acceptance tests as harness cases.
